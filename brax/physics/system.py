...
class System:
    ...
    def default_angle(self, default_index: int = 0) -> jp.ndarray:
        ...
        for joint in self.config.joints:
            if joint.name not in angles:
                angle = [(l.min + l.max) * jp.pi /
                         360 for l in joint.angle_limit]
    ...
    def default_qp(self,
        ...
        for child, parent in lineage.items():
            ...
            while parent in lineage:
                ...
                depth[child] += 1
        joint_idxs = sorted(
            joint_idxs, key=lambda x: depth.get(x[0].parent, 0))
        ...
        if joint:
            ...
            _, (local_rot, local_ang) = jp.scan(
                local_rot_ang, (), xs, len(joint))
            ...
            def set_qp(carry, x):
                ...
                world_pos = qp.pos[body_p] + \
                    math.rotate(local_pos, qp.rot[body_p])
    ...
    def info(self, qp: QP) -> Info:
        ...
        zero = P(jp.zeros((self.num_bodies, 3)),
                 jp.zeros((self.num_bodies, 3)))
    ...
    def step(self, qp: QP, act: jp.ndarray) -> Tuple[QP, Info]:
        ...
        def substep(carry, _):
            ...
            info = Info(info.contact + dp_c, info.joint +
                        dp_j, info.actuator + dp_a)
            return (qp, info), ()
